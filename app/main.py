...
# Setup logging FIRST
from app.core.utils.logging_config import setup_logging

logger = setup_logging("phn_node")
...
logger.info("Robust node synchronization initialized")
...
class RateLimiter:
    ...
    def is_allowed(self, ip: str, endpoint: str = "default") -> tuple[bool, str]:
        ...
        # Clean old requests
        self.requests[ip] = [
            t for t in self.requests[ip] if current_time - t < time_window
        ]
        ...
        if len(self.requests[ip]) >= max_requests:
            return (
                False,
                f"Rate limit exceeded: max {max_requests} requests per {time_window}s",
            )
...
def rate_limit(endpoint: str = "default"):
    ...
    def decorator(handler):
        ...
        async def wrapper(request):
            ...
            if not allowed:
                logger.warning(f"Rate limit blocked {ip} on {endpoint}: {msg}")
...
def startup_load():
    ...
    if not blockchain_loaded:
        logger.info("No blockchain found or invalid, creating genesis block")
        blockchain.clear()
        genesis = create_genesis_block()
        blockchain.append(genesis)
        save_blockchain()
        # Initialize balance cache with genesis block
        update_balance_cache(genesis)
        save_balance_cache()
    ...
    # If cache is empty but blockchain exists, rebuild it
    from app.core.blockchain.chain import balance_cache

    if len(blockchain) > 0 and len(balance_cache) == 0:
        logger.info("Balance cache empty, rebuilding from blockchain...")
        rebuild_balance_cache()
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(settings.LOGO_URL) as resp:
                    ...
                    return web.Response(
                        text=f"Failed to fetch remote logo: {resp.status}",
                        status=resp.status,
                    )
        except Exception as e:
    ...
    return web.json_response(
        {
            ...
            or request.url.with_path("/phn.png").human_repr(),
...
async def http_send_tx(request):
    ...
    with mempool_lock:
        ...
        if len(sender) == 128:  # Public key format
            ...
            logger.debug(f"Converted public key to PHN address: {balance_address}")
        else:
        ...
        logger.debug(
            f"Sender: {sender[:32]}... Balance: {balance_address} ({sender_balance} PHN), Needed: {total_needed} PHN"
        )
        ...
        logger.info(f"Transaction added to mempool: {tx['txid'][:16]}...")
        return web.json_response(
            {
                "status": "success",
                "txid": tx["txid"],
                "mempool_position": advanced_mempool.get_size(),
            }
        )
...
async def http_submit_block(request):
    ...
    # Update balance cache with new block transactions
    update_balance_cache(block)

    save_blockchain()
    save_balance_cache()  # Persist cache after each block
    ...
    valid, checkpoint_msg = chain_protection.validate_chain_against_checkpoints(
        blockchain
    )
    if not valid:
        # SECURITY ALERT: Checkpoint violation detected!
        logger.warning(f"Security: {checkpoint_msg}")
        return web.json_response(
            {
                "status": "error",
    ...
    logger.info(
        f"Block #{block['index']} accepted - Current difficulty: {current_diff}"
    )
...
try:
    ...
    logger.info("API endpoints loaded: balance, explorer, tokens, assets")
except ImportError as e:
    logger.warning(f"Could not load some API endpoints: {e}")
...
async def on_startup(app):
    ...
    for p in list(peers):
        try:
            ...
            logger.error(f"Failed to sync with {p}: {e}")
...
if __name__ == "__main__":
    print(COIN_LOGO)  # Keep logo on console for user experience
